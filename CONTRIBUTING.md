...
AI tools help write a lot of our code, docs, and issues. That's welcome. We ask only that it's disclosed, never hidden.

A commit written with AI assistance carries a `Co-authored-by` trailer naming the exact model used, for example:

```
Co-authored-by: Claude Sonnet 5 <noreply@anthropic.com>
```

A pull request or issue opened by an AI starts with a short blockquote naming the model and stating the content has not been reviewed by a human, for example:

> 🤖 Auto-generated by Claude Fable 5 (claude-fable-5) via Claude Code — NOT human-reviewed. Verify before acting.

This PR is itself an example of the second convention.

Every change goes through the same review and CI, whether AI helped write it or not. CodeRabbit also reviews every pull request automatically, on top of human review. If you use AI tools to contribute, please follow the same two conventions: name the model in a co-author trailer on the commit, and open the PR or issue with the disclosure blockquote.

This matches [NLnet's Generative AI policy](https://nlnet.nl/foundation/policies/generativeAI/). Much of the OVOS ecosystem is funded through NLnet, so we hold ourselves to it.
